function formCheck(event) {
    ...
    //원하는 값이 들어오는지 체크
    //이름 체크
    if(name.value === null || typeof name.value !== 'string' || name.value.length > 3 || /\d/.test(name.value)) {
        alert("False Name");
    }
    //번호 체크
    else if (phonevalue.toString().length < 10 || phone.value === null || isNaN(phonevalue)){
        alert("False Number");
    }
    //라디오 버튼 선택여부 체크
    else if(!job) {
